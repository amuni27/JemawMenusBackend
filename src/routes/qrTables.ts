...
router.get('/menus/:menuId/qr-tables', asyncHandler(async (req, res) => {
  const menu = await getMenuAuthorized(req.params.menuId, req.user!.vendorId);
...
router.post('/menus/:menuId/qr-tables', asyncHandler(async (req, res) => {
  const menu = await getMenuAuthorized(req.params.menuId, req.user!.vendorId);
  if (!menu) return res.status(404).json({ message: 'Menu not found' });
...
router.param('qrId',
  asyncHandler(async (req, _res, next, qrId: string) => {
